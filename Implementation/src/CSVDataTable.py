...
class CSVDataTable(BaseDataTable):
    ...
    def _add_rows(self, r):
        ...
        #     raise ValueError("Which one(s) are not clear?")
        # else:
        self._rows.append(r)
    ...
    def load(self):
        ...
        with open(fn, "r") as input_rows:
            c_reader = csv.DictReader(input_rows, dialect= 'excel')
            for r in c_reader:
                if self._column_names is None:
                    self._column_names = list(r.keys())
                if self._rows is None:
                    self._rows = []
                self._add_rows(copy.copy(r))
    ...
    def find_by_primary_key(self, key_fields, field_list=None):
        ...
        """
        temp = dict(zip(self._key_columns, key_fields))
        result = self.find_by_template(temp, field_list)
    ...
    def delete_by_key(self, key_fields):
        ...
        """
        temp = dict(zip(self._key_columns, key_fields))
        return self.delete_by_template(temp)
    ...
    def update_by_key(self, key_fields, new_values):
        ...
        temp = dict(zip(self._key_columns, key_fields))
        self.update_by_template(temp, new_values)
